 ...
 */
import Surreal, { ExperimentalSurrealHTTP, RecordId } from "surrealdb.js"
import type {
  Adapter,
  AdapterUser,
  AdapterAccount,
...
type Document<T> = Record<string, string | null | undefined> & { id: RecordId<T> }
export type UserDoc = Document<'user'> & { email: string }
...
export type SessionDoc = Document<'session'> & { userId: RecordId<'user'> }

const extractId = (surrealId: RecordId) =>
  surrealId.id
...
export function SurrealDBAdapter<T>(
  ...
  return {
    ...
    async getUser(id: string) {
      const surreal = await client
      try {
        const surrealId = new RecordId('user', id);
    ...
    async getUserByAccount({
      ...
      const surreal = await client
      try {
        const users = await surreal.query(
          `SELECT userId
           FROM account
    ...
    async updateUser(user: Partial<AdapterUser>) {
      ...
      const userRecord = new RecordId('user', user.id);
    ...
    async deleteUser(userId: string) {
      ...
      try {
        const accounts = await surreal.query<[AccountDoc[]]>(
          ...
          WHERE userId = $userId
          LIMIT 1`,
          { userId: new RecordId('user',surrealId) }
        )
        const account = accounts[0]?.[0]
        if (account) {
          const accountId = extractId(account.id)
          await surreal.delete(new RecordId('account',accountId))
        }
      } catch (e) {}
